  ...
  template: `
    ...
    >
      ...
      <div *ngIf="submitted()" class="text-center py-8">
        ...
        <h3 class="text-lg font-medium text-dark-text dark:text-white mb-2">Viewing Scheduled!</h3>
        <p class="text-light-text dark:text-gray-300 mb-6">
          Your viewing request has been submitted. Our agent will contact you to confirm the appointment.
        </p>
        <button
          type="button"
      ...
      <!-- Form State -->
      <form *ngIf="!loading() && !submitted()" #viewingForm="ngForm" (ngSubmit)="onSubmit(viewingForm)">
        <div class="space-y-4">
          <!-- Property Info -->
          ...
          <!-- Viewing Type -->
          <div>
            <label class="block text-sm font-medium text-dark-text dark:text-white mb-2">
              Viewing Type *
            </label>
            <div class="grid grid-cols-2 gap-3">
              <label class="relative">
                ...
                >
                <div 
                  class="p-3 border rounded-lg cursor-pointer transition-colors duration-200 text-center"
                  [class]="formData.viewingType === 'in-person' 
                    ? 'border-primary bg-primary/10 dark:bg-primary/20' 
                    : 'border-gray-300 dark:border-gray-600 hover:border-primary'"
                >
                  <ng-icon name="tablerHome" class="h-6 w-6 mx-auto mb-1 text-primary"></ng-icon>
                  <span class="text-sm font-medium text-dark-text dark:text-white">In-Person</span>
                </div>
              </label>
              <label class="relative">
                <input
                ...
                <div 
                  class="p-3 border rounded-lg cursor-pointer transition-colors duration-200 text-center"
                  [class]="formData.viewingType === 'virtual' 
                    ? 'border-primary bg-primary/10 dark:bg-primary/20' 
                    : 'border-gray-300 dark:border-gray-600 hover:border-primary'"
                >
                  <ng-icon name="tablerCamera" class="h-6 w-6 mx-auto mb-1 text-primary"></ng-icon>
                  <span class="text-sm font-medium text-dark-text dark:text-white">Virtual Tour</span>
                </div>
              </label>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Preferred Date -->
            <div>
              <label for="preferredDate" class="block text-sm font-medium text-dark-text dark:text-white mb-1">
                Preferred Date *
              </label>
              <input
                type="date"
            ...
            <!-- Preferred Time -->
            <div>
              <label for="preferredTime" class="block text-sm font-medium text-dark-text dark:text-white mb-1">
                Preferred Time *
              </label>
              <select
                id="preferredTime"
              ...
              <div *ngIf="time.invalid && time.touched" class="text-red-500 text-sm mt-1">
